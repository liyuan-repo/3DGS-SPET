...
if __name__ == "__main__":
    ...
    # sub_path = Path(sub_dataset)
    # sub_file = Path(sub_dataset).iterdir()
    for x in Path(sub_dataset).iterdir():
        p = x.suffix
        print(x)
        print(p)
    ...
    for six, src_tar_file in enumerate(subtar_files):
        ...
        if six % 10 == 0:
            ...
            print(f'{scene_counter}/{num_scenes}, views: {image_counter}, insts: {instance_counter}, ',
                  f'cost: {time_cost}', f'time: {time_stamp}')
