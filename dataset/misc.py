...
def recover_TxTyTz_from_delta_PxPyTz(delta_pxpy, delta_tz, camK, bbox_center, bbox_scale, zoom_scale):
    ...
    T3 = Tz[..., None].float() * (torch.inverse(camK).float() @ homo_pxpy[..., None].float()).squeeze(
        dim=2)  # Vx1 [Vx3x3, Vx3x1 => Vx3]
...
def generate_PEmap(im_hei, im_wid, cam_K):
    ...
    homo_uv = torch.stack([xx, yy, torch.ones_like(xx)], dim=0).type(torch.FloatTensor)
...
def flat_dataset_dicts_with_NN_viewpoint(dataset_dicts, visib_fract_threshold=None, min_covisib_viewpoint=15,
                                         max_covisib_viewpoint=60):
    """
    flatten the dataset dicts of detectron2 format
    original: list of dicts, each dict contains some image-level infos
              and an "annotations" field for instance-level infos of multiple instances
    ...
    for data_entry in object_lists:
        ...
        covisib_viewpoint_inds = (viewpoint_deg_dists >= min_covisib_viewpoint) & (
                viewpoint_deg_dists <= max_covisib_viewpoint)
...
def torch_find_connected_component(mask, return_bbox=True, min_bbox_scale=14, include_supmask=True):
    ...
    if return_bbox:
        ...
        det_masks = torch.stack(det_masks, dim=0).type(torch.FloatTensor)
        det_scales = torch.stack(det_scales, dim=0).type(torch.FloatTensor)
        det_centers = torch.stack(det_centers, dim=0).type(torch.FloatTensor)
