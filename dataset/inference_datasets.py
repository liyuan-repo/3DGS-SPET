...
class LINEMOD_Dataset_GEN6D(torch.utils.data.Dataset):
    def __init__(self, data_root, obj_name,
        ...
        self.diameter = gs_utils.read_numpy_data_from_txt(
            os.path.join(self.obj_dir, 'distance.txt')).squeeze() / 100.0  # in meter
        ...
        for idx, img_inst in enumerate(image_subset_lists):
            ...
            if self.load_gt_bbox:
                gt_x1, gt_y1, gt_x2, gt_y2 = mmcv.load(
                    os.path.join(self.obj_dir, 'bboxes', f'bbox{int(image_ID)}.json'))
...
class LINEMOD_Dataset_BOP(torch.utils.data.Dataset):
    def __init__(self, data_root, obj_name,
        ...
        self.obj_bbox3d = np.array([
            [-bbox3d_xyz[0], -bbox3d_xyz[0], -bbox3d_xyz[0], -bbox3d_xyz[0], bbox3d_xyz[0], bbox3d_xyz[0],
             bbox3d_xyz[0], bbox3d_xyz[0]],
            [-bbox3d_xyz[1], -bbox3d_xyz[1], bbox3d_xyz[1], bbox3d_xyz[1], -bbox3d_xyz[1], -bbox3d_xyz[1],
             bbox3d_xyz[1], bbox3d_xyz[1]],
            [-bbox3d_xyz[2], bbox3d_xyz[2], bbox3d_xyz[2], -bbox3d_xyz[2], -bbox3d_xyz[2], bbox3d_xyz[2], bbox3d_xyz[2],
             -bbox3d_xyz[2]]
        ]).T / 2
        ...
        for idx, img_inst in enumerate(image_subset_lists):
            ...
            obj_pose[:3, 3] = np.array(pose_RT['cam_t_m2c'], dtype=np.float32).reshape(
                3) * self.to_meter_scale  # convert to m
            ...
            if self.load_yolo_det:
                yolo_bbox_path = os.path.join(obj_yolo_label_dir,
                                              '{:06d}.txt'.format(image_ID + 1))  # yolo_results starts from 1
...
class LOWTEXTUREVideo_Dataset(torch.utils.data.Dataset):
    def __init__(self, data_root, obj_name, subset_mode='train', num_grid_points=4096, obj_database_dir=None,
        ...
        assert (subset_mode in ['train', 'test']), f'{subset_mode} is not a valid subset mode [train, test]'
        assert (
                    obj_name in LOWTEXTUREVideo_OBJECTS.keys()), f'{obj_name} is not in the LowTexture {LOWTEXTUREVideo_OBJECTS.keys()}'
        self.subset_mode = subset_mode
        self.data_root = data_root
        self.obj_name = obj_name
        self.num_grid_points = num_grid_points
        ...
        if os.path.exists(self.obj_cad_model_path):
            ...
            try:
                self.diameter = float(
                    np.loadtxt(os.path.join(LOWTEXTURE_SCNNED_OBJEJCT_PATH, self.obj_dir_name, 'diameter.txt')))
            except:
