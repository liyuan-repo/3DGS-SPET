...
for iter_steps in range(1, max_steps + 1):
    ...
    with autocast(enable_FP16_training):
        ...
        flops, params = profile(model_net, inputs=(batch_data,))
        # output
    ...
    with torch.no_grad():
        ...
        if ((iter_steps > 5 and iter_steps < 2000 and iter_steps % short_log_interval == 0)
            ...
            rgb_que_image = batch_data['query_dict']['rescaled_image'][0].detach().cpu().permute(1, 2,
                                                                                                 0).squeeze().float()
            gt_que_full_mask = batch_data['query_dict']['rescaled_mask'][0].detach().cpu().permute(1, 2,
                                                                                                   0).squeeze().float()
            ...
            for vix in range(vis_num_views - 1):
                vjx = vix + 2
                rgb_ref_image = batch_data['refer_dict']['zoom_image'][vix].detach().cpu().permute(1, 2,
                                                                                                   0).squeeze().float()
                gt_ref_mask = batch_data['refer_dict']['zoom_mask'][vix].detach().cpu().permute(1, 2,
                                                                                                0).squeeze().float()
