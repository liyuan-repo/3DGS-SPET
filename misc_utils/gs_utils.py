...
def GS_zoom_and_crop(image, K, t, radius, target_size=224, margin=0, mode='bilinear'):
    ...
    image_new = torch.nn.functional.grid_sample(image.type(torch.FloatTensor),
                                                grids.type(torch.FloatTensor),
...
def zoom_in_and_crop_with_offset(image, K, t, radius, target_size=224, margin=0, mode='bilinear'):
    ...
    cam_focal = K[:, :2, :2].max()  # 相当(近似)于相机焦距f
    bbox_scale = 2 * (1 + margin) * cam_focal * radius.to(K.device) / t[:, 2].to(K.device)
    # print("bbox_scale is", bbox_scale)
    # bbox_scale/cam_focal = 2*radius/t[:, 2]
    rescaling_factor = target_size / bbox_scale  # 缩放因子

    Smat = torch.eye(3, device=K.device)[None, ...].repeat(K.size(0), 1, 1)
    Smat[:, 0, 0] *= rescaling_factor
    Smat[:, 1, 1] *= rescaling_factor
    zoom_camK = torch.einsum('nij,njk->nik', Smat, K)
    zoom_camK[:, :2, 2] = target_size / 2    # zoom_camK的焦距f

    bbox_uvs = torch.einsum('nij,nj->ni', K, t)
    bbox_center = bbox_uvs[:, :2] / bbox_uvs[:, 2:3]
    # print('bbox_scale is', bbox_center)
    ...
    image_new = torch.nn.functional.grid_sample(
        image.type(torch.float32), grids.type(torch.FloatTensor), mode=mode, align_corners=True)
    image_new = image_new.permute(0, 2, 3, 1)
...
def draw_3d_bounding_box(rgb_image, projected_bbox, color, linewidth=3):
    ...
    # Define the edges of the bounding box by connecting the vertices 顶点
...
def draw_xyz_axis(color, ob_in_cam, scale=0.1, K=np.eye(3), thickness=3, transparency=0, is_input_rgb=False):
    ...
    """
    if is_input_rgb:
        color = cv2.cvtColor(color, cv2.COLOR_RGB2BGR)  # convert an image from one color space to another
    ...
    tmp = color.copy()
    tmp1 = tmp.copy()
    tmp1 = cv2.arrowedLine(tmp1, origin, xx, color=(0, 0, 255), thickness=thickness, line_type=line_type,
                           tipLength=arrow_len)
    mask = np.linalg.norm(tmp1 - tmp, axis=-1) > 0
    tmp[mask] = tmp[mask] * transparency + tmp1[mask] * (1 - transparency)
    tmp1 = tmp.copy()
    tmp1 = cv2.arrowedLine(tmp1, origin, yy, color=(0, 255, 0), thickness=thickness, line_type=line_type,
                           tipLength=arrow_len)
    mask = np.linalg.norm(tmp1 - tmp, axis=-1) > 0
    tmp[mask] = tmp[mask] * transparency + tmp1[mask] * (1 - transparency)
    tmp1 = tmp.copy()
    tmp1 = cv2.arrowedLine(tmp1, origin, zz, color=(255, 0, 0), thickness=thickness, line_type=line_type,
                           tipLength=arrow_len)
    mask = np.linalg.norm(tmp1 - tmp, axis=-1) > 0
    tmp[mask] = tmp[mask] * transparency + tmp1[mask] * (1 - transparency)
