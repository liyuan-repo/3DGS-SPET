...
from misc_utils.metric_utils import get_score
...
if __name__ == '__main__':
    ...
    for frame_idx in range(len(test_poses)):
        ...
        # -----------------GT bbox3d-------------------------------
        gt_RT = torch.as_tensor(gt_pose_RT, dtype=torch.float32)  # 外参矩阵[R|t] 4×4
        ...
        query_bbox3d_frame = gs_utils.draw_3d_bounding_box(query_bbox3d_frame, gt_bbox_pts,
                                                           color=color_gt, linewidth=12)
        mix_bbox3d_frame = gs_utils.draw_3d_bounding_box(query_bbox3d_frame, pre_bbox_pts,
                                                         color=color_3dgs, linewidth=5)
        bbox_3D_img = os.path.join(bbox_3D_path, f'{query_imgs[frame_idx]}')
        cv2.imwrite(bbox_3D_img, mix_bbox3d_frame)

        T_error, R_error_deg, esa_score = np.round(get_score(pre_pose, gt_pose_RT), 2)
        # print(T_error, R_error_deg, esa_score)
        print(frame_idx)
        all_error = [T_error, R_error_deg, esa_score]
        all_errors.append(all_error)

        if (frame_idx + 1) % 30 == 0:
            print('[{}/{}], \t refining_step:{}, \t {:.1f} FPS'.format(
                frame_idx + 1, len(query_imgs), iter_step, (frame_idx - start_idx) / gsp_accum_runtime))

    filename = os.path.join(demo_data_dir, f'pose_error_{obj_name}.csv')
    pose_error = DataFrame(all_errors, columns=['T_error', 'R_error_deg', 'esa_score'])
