...
def create_reference_database_from_RGB_images(model_func, obj_dataset, device, save_pred_mask=False):
    if CFG.USE_ALLOCENTRIC:  # to obtain normalized object-centric images 获得以物体为中心的图像  USE_ALLOCENTRIC=True
    ...
    # -------1) Given Nr reference images, we first select Nk (≪ Nr) keyframes using farthest point sampling (FPS) [39]
    # based on their corresponding 3D rotation labels      # refer_view_num = 8
    ...
    for ref_idx in fps_inds:
        ...
        # camK = datum['camK'].squeeze(dim=0)  # 3x3
        camK = datum['camK']  # 3x3
    ...
    ref_fps_images = torch.stack(ref_fps_images, dim=0)  # Nk (≪ Nr) keyframes
    ...
    zoom_fps_images = gs_utils.zoom_in_and_crop_with_offset(image=ref_fps_images,  # KxHxWx3 -> KxSxSx3
                                                            ...
                                                            target_size=CFG.zoom_image_scale,  # 224
                                                            margin=CFG.zoom_image_margin)['zoom_image']  # 0

    with torch.no_grad():
        if zoom_fps_images.shape[-1] == 3:
            zoom_fps_images = zoom_fps_images.permute(0, 3, 1, 2)

        # 2) Then, we extract image/(keyframe) feature tokens Ffps ∈ R Nk×L×C from these keyframes using DINOv2,
        # where L and C denote the token number and feature dimension of each frame
        # obj_fps_feats (Ffps) ——> keyframe feature tokens,  obj_fps_dino_tokens ——> semantic feature tokens
        obj_fps_feats, _, obj_fps_dino_tokens = model_func.extract_DINOv2_feature(zoom_fps_images.to(device),
                                                                                  return_last_dino_feat=True)
        # obj_fps_feats与obj_fps_dino_tokens之间是什么样的关系？
        # extract_DINOv2_feature return dino_feat, dino_feat_clstoken, dino_feat_patchtokens

        # 3) Next, we feed these feature tokens into the Co-Segmenter, consisting of a transformer-like module
        # and a mask decoding head, to jointly predict the object segmentation masks.

        # or The transformed Ffps is then fed into the mask decoding head to produce the keyframe segmentation masks
        obj_fps_masks = model_func.refer_cosegmentation(obj_fps_feats).sigmoid()  # Kx1xSxS     # Kx768x16x16

        # dino_patch_size = 14
        obj_token_masks = torch_F.interpolate(obj_fps_masks,
                                              scale_factor=1.0 / model_func.dino_patch_size,
                                              mode='bilinear', align_corners=True,
                                              recompute_scale_factor=True)  # Kx1xS/14xS/14

        # 4) Finally, we extract the object-aware semantic feature tokens (obj_fps_dino_tokens) from the keyframe
        # feature maps Ffps using the predicted masks.
        obj_fps_dino_tokens = obj_fps_dino_tokens.flatten(0, 1)[obj_token_masks.view(-1).round().type(torch.bool), :]
        # KxLxC -> KLxC -> MxC

    # --------------------save_pred_mask-----------------
    refer_allo_Rs = list()
    refer_pred_masks = list()
    refer_Remb_vectors = list()  # rotation-aware embedding vectors
    refer_coseg_mask_info = list()
    num_instances = len(obj_dataset)

    # 3.1.2小节 (Rotation-Aware Representation Encoding)
    # we first adopt an Obj-Segmenter to segment the object from each reference image and then utilize
    # a Rotation-Aware Encoder (RA-Encoder) to extract an image-level embedding vector from the segmented image
    # Obj-Segmenter includes the DINOv2 backbone, a transformer-like module, and a mask decoding head
    # (identical to the one in Co-Segmenter)

    # 58行到110行是关键帧图像的特征提取，从以下开始，是所有参考图像的处理
    for idx in range(num_instances):
        ...
        with torch.no_grad():
            ...
            # 1) Obj-Segmenter first extracts the DINOv2 feature tokens (zoom_feat) from the reference image.
            zoom_feat = model_func.extract_DINOv2_feature(zoom_image.to(device))
            # 2) the reference image feature tokens (zoom_feat) along with the keyframe feature /object semantic tokens
            # (obj_fps_feats) are fed into the transformer-like module to perform self and cross-attention computation.
            # Subsequently, the mask decoding head is utilized to produce the segmentation masks (zoom_mask) from
            # the transformed image features (zoom_feat) (see Fig. 3 middle).
            zoom_mask = model_func.query_cosegmentation(x_que=zoom_feat,
                                                        x_ref=obj_fps_feats,
                                                        ref_mask=obj_fps_masks).sigmoid()
            # Finally, we extract an image-level representation vector (3D object rotation-aware embedding vectors)
            # Viref ∈ R64 from the segmented RGB image using RA-Encoder.
            # 从参考图像特征reference image DINOv2 feature和对应的分割掩膜中the segmentation masks提取三维感知嵌入向量
            y_Remb = model_func.generate_rotation_aware_embedding(zoom_feat, zoom_mask)
            refer_Remb_vectors.append(y_Remb.squeeze(0))  # 64
            # Finally, we extract the 3D rotation-aware embedding vectors from the reference image features and
            # their corresponding segmentation masks

            try:
                # torch.nonzero：用于输出数组的非零值的索引，即用来定位数组中非零的元素
                msk_yy, msk_xx = torch.nonzero(zoom_mask.detach().cpu().squeeze().round().type(torch.uint8),
                                               as_tuple=True)
                # (msk_cx,msk_cy), 查询图像mask的中心位置
            ...
            # 如果运行try中的代码没有给出异常，将会跳过except代码块。否则会给出异常对应的代码块，并执行其中的代码。
            # 如果try执行出现错误时，用户将会看到友好的提示信息，而不是traceback

            # detach意为分离，对某个张量调用函数detach()的作用是返回一个Tensor，它和原张量的数据相同，但requires_grad=False
            # 也就意味着detach得到的张量不会具有梯度。
        ...
        if save_pred_mask:  # save_pred_mask=True
    ...
    ref_database = dict()
    if not save_pred_mask:  # save_pred_mask=True
    ...
    ref_database['obj_fps_feats'] = obj_fps_feats  # keyframe feature tokens (Ffps)
    ref_database['obj_fps_masks'] = obj_fps_masks
    ref_database['obj_fps_dino_tokens'] = obj_fps_dino_tokens  # semantic feature tokens (Fobj)

    ref_database['refer_allo_Rs'] = refer_allo_Rs
    ref_database['refer_Remb_vectors'] = refer_Remb_vectors  # rotation-aware embedding vectors
...
def perform_segmentation_and_encoding(model_func, que_image, ref_database, device):
    with torch.no_grad():
        ...
        # que_image dim is 4 and torch.Size([1, 3, 672, 504])
        if que_image.dim() == 3:
            que_image = que_image.unsqueeze(0)
        if que_image.shape[-1] == 3:
            que_image = que_image.permute(0, 3, 1, 2)

        # 1) extract the DINOv2 feature tokens (que_feats) from query images and then compute the image-level cosine
        # similarities between these image features and the object semantic representation.
        que_feats = model_func.extract_DINOv2_feature(que_image.to(device))

        # 2) Detector first employs an Obj-Segmenter to produce a segment mask from the query image using
        # the semantic information (Fobj)
        ...
        mask_threshold = CFG.coarse_threshold  # 0.05

        # 3) from the query image segment mask we generate a set of mask proposals by finding the connected components,
        # Subsequently, a set of object-centric RGB images {Iique}m i=1 are cropped from the input image using the
        # 2D bounding boxes derived from these mask proposals.
        while True:
            ...
            if que_binary_mask.sum() < CFG.DINO_PATCH_SIZE ** 2:  # DINO_PATCH_SIZE = 14
                mask_threshold -= 0.01
                continue
            else:
                break
        _, pd_coarse_tight_scales, pd_coarse_centers = misc.torch_find_connected_component(
            que_binary_mask, include_supmask=CFG.CC_INCLUDE_SUPMASK, min_bbox_scale=CFG.DINO_PATCH_SIZE,
            return_bbox=True)

        pd_coarse_tight_scales = pd_coarse_tight_scales.cuda()  # devices=cuda
        pd_coarse_centers = pd_coarse_centers.cuda()

        pd_coarse_scales = pd_coarse_tight_scales * CFG.coarse_bbox_padding  # coarse_bbox_padding = 1.25
        ...
        # a set of object-centric RGB images of size 224×224
        # ops.roi_align是一种在计算机视觉中常用的操作，用于将感兴趣区域 (Region of Interest, ROI) 对齐到固定大小的特征图上。
        # 该操作通常用于目标检测、目标识别和姿态估计等任务中。它在torchvision做成了一个接口：
        # 假设候选框坐标为左上角(0,105)，右下角：(230，250)，原图和featureMap的spaceRatio为32，那么映射到featureMap上的候选框为：
        # 左上角：(0，105/32)，即为(0，3.28125)；右下角：(230/32，250/32),即为(7.1875,7.8125)，那么候选框在特征图上的区域即为
        # 下图中红色区域。注意，这里并没有对坐标进行取整，因此是精确的坐标。
        ...
        # boxes：感兴趣区域的坐标，形状为 (R, 5)，其中R是感兴趣区域的数量。每个感兴趣区域由[batch_index, x1, y1, x2, y2] 组成，
        # 其中batch_index是感兴趣区域所属的批量索引，(x1, y1) 是感兴趣区域左上角的坐标，(x2, y2) 是感兴趣区域右下角的坐标。

        # feed these RGB images into the proposal selector to obtain the final detection result.
        # Within the proposal selector, we first extract the DINOv2 feature tokens {Fique ∈ RL×C} from
        # these cropped images and then compute the image-level cosine similarities between these image features
        # and the object semantic representation Fobj. We select the one with the highest similarity score as the output
        if roi_RGB_crops.shape[0] == 1:
            rgb_img_crop = roi_RGB_crops  # 1x3xSxS
            rgb_box_scale = pd_coarse_scales.squeeze(0)  # coarse_scales = tight_scales * 1.25
            ...
            roi_img_feats, _, roi_dino_tokens = model_func.extract_DINOv2_feature(roi_RGB_crops,
                                                                                  return_last_dino_feat=True)
            roi_img_masks = model_func.query_cosegmentation(x_que=roi_img_feats,
                                                            x_ref=ref_database['obj_fps_feats'],
                                                            ref_mask=ref_database[
                                                                'obj_fps_masks']).sigmoid()  # KxCxSxS -> Kx1xSxS
            ...
            if CFG.cosim_topk > 0:
                ...
                cosim_score = token_cosim.mean(dim=-1).sum(dim=1) / (
                        1 + roi_obj_mask.squeeze(-1).sum(dim=1))  # KxLxN -> KxL -> K
        ...
        if CFG.enable_fine_detection:  # True
            mask_threshold = CFG.finer_threshold  # 0.5
            while True:
            ...
            _, pd_fine_tight_scales, pd_fine_centers = misc.torch_find_connected_component(
                fine_binary_mask, include_supmask=CFG.CC_INCLUDE_SUPMASK, min_bbox_scale=CFG.DINO_PATCH_SIZE,
                return_bbox=True)

            pd_fine_tight_scales = pd_fine_tight_scales.cuda()  # devices=cuda
            pd_fine_centers = pd_fine_centers.cuda()
            ...
            if roi_RGB_crops.shape[0] == 1:
                ...
                roi_obj_mask = torch_F.interpolate(roi_img_masks,
                                                   scale_factor=1.0 / CFG.DINO_PATCH_SIZE,
                                                   mode='bilinear', align_corners=True,
                                                   recompute_scale_factor=True).flatten(2).permute(0, 2,
                                                                                                   1).round()  # KxLx1
                roi_dino_tokens = roi_obj_mask * roi_dino_tokens
                token_cosim = torch.einsum('klc,nc->kln', torch_F.normalize(roi_dino_tokens, dim=-1),
                                           torch_F.normalize(ref_database['obj_fps_dino_tokens'], dim=-1))
                if CFG.cosim_topk > 0:
                    cosim_score = token_cosim.topk(dim=1, k=CFG.cosim_topk).values.mean(dim=-1).mean(
                        dim=1)  # KxLxN -> KxTxN -> KxN -> K
                else:
                    cosim_score = token_cosim.mean(dim=-1).sum(dim=1) / (
                            1 + roi_obj_mask.squeeze(-1).sum(dim=1))  # KxLxN -> KxL -> K
    ...
    return {
        ...
        'rgb_bbox': pd_fine_bboxes.squeeze(0),  # 2D边界框
...
def perform_segmentation_and_encoding_from_bbox(model_func, que_image, ref_database, device):
    with torch.no_grad():
        ...
        rgb_crop_mask = model_func.query_cosegmentation(x_que=rgb_img_feat,
                                                        x_ref=ref_database['obj_fps_feats'],
                                                        ref_mask=ref_database[
                                                            'obj_fps_masks']).sigmoid()  # 1xCxSxS -> 1x1xSxS
        obj_Remb_vec = model_func.generate_rotation_aware_embedding(rgb_img_feat, rgb_crop_mask)
        RAEncoder_cost = time.time() - RAEncoder_timer
...
# We first obtain a segmentation mask Mque using Obj-Segmenter as well as an image-level representation vector Vque ∈
# R64 using RA-Encoder from the detection. We then input Mque and V que into the pose decoder to compute an initial
# 6D pose Pinit

def multiple_initial_pose_inference(obj_data, ref_database, device):
    camK = obj_data['camK'].to(device).squeeze()  # torch.squeeze:降维，去除size为1的维度
    obj_Remb = obj_data['obj_Remb'].to(device).squeeze()  # query image Rotation-Aware embedding vector
    obj_mask = obj_data['rgb_mask'].to(device).squeeze()  # query image segmentation mask
    bbox_scale = obj_data['bbox_scale'].to(device).squeeze()
    bbox_center = obj_data['bbox_center'].to(device).squeeze()

    # the pose decoder first computes the cosine similarity scores between the query RA vector (Vque) and
    # the reference RA vectors. Consequently, the reference rotation matrix (Rjref) with the highest
    # similarity score is retrieved as the initial 3D rotation estimate (Rinit)
    ...
    # We then infer the initial 3D translation Tinit using the query mask (Mque) and the jth reference mask (Mjref).
    selected_nnb_info = ref_database['refer_coseg_mask_info'][max_inds]  # Kx4   第j个 reference mask
    # 其中 refer_coseg_mask_info.append(torch.tensor([msk_cx, msk_cy, ref_tz, bin_mask_area, prob_mask_area]))
    nnb_ref_Cx = selected_nnb_info[:, 0]  # the 2D center of the retrieved reference mask
    nnb_ref_Cy = selected_nnb_info[:, 1]  #
    nnb_ref_Tz = selected_nnb_info[:, 2]  # the retrieved object reference distance
    nnb_ref_bin_area = selected_nnb_info[:, 3]
    nnb_ref_prob_area = selected_nnb_info[:, 4]  # the retrieved reference mask

    # torch.nonzero：用于输出数组的非零值的索引，即用来定位数组中非零的元素
    que_msk_yy, que_msk_xx = torch.nonzero(obj_mask.round().squeeze(), as_tuple=True)
    que_msk_cx = (que_msk_xx.max() + que_msk_xx.min()) / 2   # (msk_cx,msk_cy) 查询图像mask的中心位置
    que_msk_cy = (que_msk_yy.max() + que_msk_yy.min()) / 2
    que_bin_msk_area = obj_mask.round().sum()
    que_prob_msk_area = obj_mask.sum()  # the query reference mask

    if CFG.BINARIZE_MASK:   # False
        delta_S = (que_bin_msk_area / nnb_ref_bin_area) ** 0.5
    else:
        # calculate a relative scale factor between query mask and the retrieved reference mask
        delta_S = (que_prob_msk_area / nnb_ref_prob_area) ** 0.5

    # calculate a relative 2D center offset ratio between query mask and the retrieved reference mask
    ...
    # compute the object distance in the query image
    que_Tz = nnb_ref_Tz / delta_S * CFG.zoom_image_scale / bbox_scale  #

    # compute the 2D center in the query image
...
def multiple_refine_pose_with_GS_refiner(obj_data, init_pose, gaussians, device):
    def GS_Refiner(image, mask, init_camera, gaussians, return_loss=False):
        ...
        for iter_step in range(CFG.MAX_STEPS):
            ...
            if iter_step >= CFG.EARLY_STOP_MIN_STEPS:
                ...
                if loss_grads[
                   -CFG.EARLY_STOP_MIN_STEPS:].mean() < CFG.EARLY_STOP_LOSS_GRAD_NORM:  # early stop the refinement
                    break
        ...
        if return_loss:
            sobel_x = torch.tensor([[1, 0, -1], [2, 0, -2], [1, 0, -1]], dtype=torch.float32).view(1, 1, 3, 3).repeat(1,
                                                                                                                      3,
                                                                                                                      1,
                                                                                                                      1)
    ...
    if CFG.APPLY_ZOOM_AND_CROP:  # True
...
def eval_GSPose_with_database(model_func, obj_dataset, reference_database, output_pose_dir=None, save_pred_mask=False):
    ...
    for view_idx in range(num_que_views):
        ...
        try:
            if CFG.USE_YOLO_BBOX:
                ...
                if save_pred_mask:
                    ...
                    rgb_crop = (obj_data['rgb_image'].detach().cpu().permute(1, 2, 0) * 255).numpy().astype(np.uint8)[:,
                               :, ::-1]
                ...
                if save_pred_mask:
                    ...
                    rgb_crop = (obj_data['rgb_image'].detach().cpu().squeeze().permute(1, 2, 0) * 255).numpy().astype(
                        np.uint8)[:, :, ::-1]
                    cv2.imwrite(coseg_mask_path.replace('.png', '_rgb.png'), rgb_crop)

            obj_data['camK'] = camK
            # object sequence-wise camera intrinsics, e.g., a fixed camera intrinsics for all frames within a sequence
            obj_data['img_scale'] = max(que_hei, que_wid)

            initilizer_timer = time.time()
            init_RTs = multiple_initial_pose_inference(obj_data=obj_data, ref_database=reference_database,
                                                       device=device)
            init_RT = init_RTs[0]
            ...
            if args.enable_GS_Refiner:
                ...
                refiner_oupt = multiple_refine_pose_with_GS_refiner(obj_data, init_pose=init_RTs,
                                                                    gaussians=obj_gaussians, device=device)
        ...
        if (view_idx + 1) % 100 == 0 or (view_idx + 1) == num_que_views:
            ...
            print(
                '[{}/{}], init=[{}], refine=[{}], {}'.format(view_idx + 1, num_que_views, init_log_str, refine_log_str,
                                                             time_stamp))
            ...
            log_runtime = 'detector_cost: {:.4f}, initilizer_cost: {:.4f}, refiner_cost: {:.4f}, ' \
                          'total_cost: {:.4f}'.format(det_cost, init_cost, refine_cost, total_cost)
            # print('[{}/{}], {}'.format(view_idx+1, num_que_views, log_runtime))

            log_file_f.write(
                '[{}/{}], init=[{}], refine=[{}], {}, {}\n'.format(view_idx + 1, num_que_views, init_log_str,
                                                                   refine_log_str, time_stamp, log_runtime))
...
if __name__ == "__main__":
    ...
    parser.add_argument('--database_dir', default='LM_database', type=str, help='reference database directory')
    parser.add_argument('--outpose_dir', default='LM_yolo_pose', type=str, help='output pose directory')
    ...
    if CFG.BINARIZE_MASK:  # 不使用二值化的mask
        postfix = 'binamask'
    else:
        postfix = 'probmask'
    ...
    for obj_name, obj_dir_name in datasetObjects.items():
        ...
        if not os.path.exists(obj_ref_database_path):
            ...
            obj_refer_dataset = datasetLoader(data_root, obj_name,
                                              ...
                                              obj_database_dir=obj_refer_database_dir)
            ref_database = create_reference_database_from_RGB_images(model_net, obj_refer_dataset, device=device,
                                                                     save_pred_mask=True)
