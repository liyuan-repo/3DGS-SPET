...
def readCameras(dataloader, zoom_scale=512, margin=0.0, frame_sample_interval=1):
    ...
    # bbox3d_diameter = np.linalg.norm(obj_bbox3D_dims)  # 3D包围框的直径。
    ...
    for frame_idx in range(len(dataloader)):
        ...
        if 'image_path' not in obj_data:    # if execute
            image_path = None
...
def readObjectInfo(train_dataset, test_dataset, model_path, num_points=4096, zoom_scale=512, margin=0.0,
                   random_points3D=False):
    ...
    test_cam_infos = readCameras(test_dataset, zoom_scale=zoom_scale, margin=margin,
                                 frame_sample_interval=test_interval)
    ...
    if not random_points3D:
        ...
        # np.prod(obj_bbox3D_dims, axis=0) 表示bbox3D的体积，将整个体积等分为num_points个正方体grid，其大小为grid_cube_size
