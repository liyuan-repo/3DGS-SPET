...
from gaussian_object.utils.graphics_utils import getWorld2View2, getProjectionMatrix
...
class Camera(nn.Module):
    def __init__(self, colmap_id, R, T, FoVx, FoVy, image, gt_alpha_mask,
        ...
        # self.world_view_transform = torch.tensor(getWorld2View2(self.R, self.T, self.trans, self.scale))
        # .transpose(0, 1).cuda()
    ...
    def projection_matrix(self):
        return getProjectionMatrix(znear=self.znear, zfar=self.zfar,
                                   fovX=self.FoVx, fovY=self.FoVy,
                                   cx_offset=self.cx_offset,
                                   cy_offset=self.cy_offset,
                                   ).transpose(0, 1).cuda()
