...
from model.blocks import PositionGetter
from model.position_encoding import RoPE2D, PositionEncodingSine2D
from model.generalized_mean_pooling import GeM2D as GeMeanPool2d
from model.dino_layers import (MemEffSelfAttention,
...
class model_arch(nn.Module):
    def __init__(self, cfg=None):
        ...
        # 该函数用于返回指定键key的值，如果键不存在，则返回默认值None。
        ...
        # self.dino_backbone = torch.load(os.path.join(PROJ_ROOT, 'checkpoints/dinov2_vitb14_pretrain.pth'))
        self.dino_backbone = torch.hub.load('/home/cqupt/.cache/torch/hub/facebookresearch_dinov2_main',
                                            'dinov2_vitb14', trust_repo=True, source='local')
        ...
        self.coseg_aware_projection = nn.Sequential(
            nn.Conv2d(self.backbone_feat_dim, self.coseg_feat_dim, (1, 1), (1, 1), 0),
        ...
        # Return True if pos_embed starts with the specified prefix, False otherwise.
        elif self.pos_embed.startswith('RoPE'):  # eg RoPE100
            self.coseg_position_encoding = None  # nothing to add in the encoder with RoPE
            if RoPE2D is None:
                raise ImportError("Cannot find cuRoPE2D, please install it")
        ...
        self.coseg_prediction_head = nn.Sequential(
            nn.Conv2d(self.coseg_feat_dim, self.coseg_feat_dim, (3, 3), (1, 1), 1),
            nn.GroupNorm(1, self.coseg_feat_dim),
            nn.LeakyReLU(inplace=True),
            nn.Conv2d(self.coseg_feat_dim, 49, (3, 3), (1, 1), 1),
        )

        self.pose_aware_projection = nn.Sequential(
            nn.Conv2d(self.backbone_feat_dim, self.pose_feat_dim, (1, 1), (1, 1), 0),
            nn.GroupNorm(1, self.pose_feat_dim),
            nn.LeakyReLU(inplace=True),
        )
        # four 3×3 convolutional layers with stride 1, a generalized average pooling layer,
        # and a fully connected layer with an output dimension of 64
        self.rotation_embedding_head = nn.Sequential(
            nn.Conv2d(self.pose_feat_dim + 1, 128, (3, 3), (1, 1), 1),  # Cx32x32 -> 128x32x32
            nn.GroupNorm(1, 128),
            nn.LeakyReLU(inplace=True),
            nn.Conv2d(128, 256, (3, 3), (2, 2), 1),  # 128x32x32 -> 256x16x16
            nn.GroupNorm(1, 256),
            nn.LeakyReLU(inplace=True),

            nn.Conv2d(256, 128, (3, 3), (1, 1), 1),  # 256x16x16 -> 128x16x16
            nn.GroupNorm(1, 128),
            nn.LeakyReLU(inplace=True),
            nn.Conv2d(128, 256, (3, 3), (2, 2), 1),  # 128x16x16 -> 256x8x8
            nn.GroupNorm(1, 256),
            nn.LeakyReLU(inplace=True),
    ...
    def extract_DINOv2_feature(self, x, return_last_dino_feat=False):
        ...
        dim_h = dim_H // self.dino_patch_size  # DINO_PATCH_SIZE = 14
        ...
        if return_last_dino_feat:
            ...
            dino_feat = self.dino_backbone.norm(x)
            dino_feat_norm_clstoken = dino_feat[:, 0, :]  # BxC
            dino_feat_norm_patchtokens = dino_feat[:, 1:, :]  # BxLxC
            return xs, dino_feat_norm_clstoken, dino_feat_norm_patchtokens
        return xs
    ...
    def refer_cosegmentation(self, x):
        ...
        # we reshape the keyframe feature tokens as feature maps (denoted as Fˆfps), from which we sample a set of
        # frame-wise center tokens Fˆfps c ∈ RNk×C located at the 2D center of these feature maps.
        ...
        # 是用于从输入特征图中采样的函数.在输入张量中按照网格坐标采样，生成一个新的特征图。
        # 给定维度为(N,C,H_in,W_in) 的input特征图，维度为(N,H_out,W_out,2) 的grid，则该函数output的维度为(N,C,H_out,W_out)。
        # gird(N,H_out,W_out,2)的作用在于提供一批用于在输入特征图上进行元素采样的位置坐标。grid的元素值通常在[-1,1]之间，(-1,-1)
        # 表示取输入特征图左上角的元素，(1,1) 表示取输入特征图右下角的元素。
        # output(N,C,H_out,W_out)表示函数输出的一批特征图，其批大小依然为N，特征图的通道数依然为C，但特征图的高已经变成了H_out，
        # 宽变成了W_out，并且输出特征图中的元素值是从根据grid所提供的位置坐标在输入特征图中采样得到的。

        # https://blog.csdn.net/qq_40968179/article/details/128093033
        # https://blog.csdn.net/weixin_41496173/article/details/139607685
        ...
        # The transformed Ffps is then fed into the mask decoding head (two 3×3 convolutional layers followed by
        # an upsampling layer) to produce the keyframe segmentation masks
        x = self.coseg_refer_LayerNorm(x)  # BxVrLxC    是归一化层 nn.LayerNorm
        x = x.reshape(dim_BVr, dim_S, dim_S, -1).permute(0, 3, 1, 2)  # BVrxLxC -> BVrxCxSxS

        # ------mask decoding head-------
        x = self.coseg_prediction_head(x)  # BVrxCx32x32 -> BVrx2x32x32  ----have two 3×3 convolutional layers
        x = self.pixelshuffle_x7(x)  # BVrx49x32x32 -> BVrx1x224x224
        return x
        # Pixel Shuffle 是一种常用的上采样技术，其工作原理是通过一个称为 sub-pixel convolution 的过程来实现上采样
        # Pixel Shuffle是一种从特征图中提取空间信息的方法，主要应用于图像超分辨率等任务。
        # Pixel Shuffle是一种将通道维度转换为空间维度的操作，用于将特征图从较低的空间分辨率上采样到较高的分辨率。
    ...
    def query_cosegmentation(self, x_que, x_ref, ref_mask, enable_mask_filter=False):
        ...
        if enable_mask_filter:
            ...
            x_ref = x_ref[mask_inds.repeat(1, 1, x_ref.shape[-1])].reshape(dim_B, -1,
                                                                           x_ref.shape[-1])  # BxVrLxC -> BxMxC
            ref_pos = ref_pos[mask_inds.repeat(1, 1, ref_pos.shape[-1])].reshape(dim_B, -1,
                                                                                 ref_pos.shape[-1])  # BxVrLx2 -> BxMx2
        ...
        for lix in range(self.coseg_num_layers):
            ...
            x_que = self.coseg_query_crossAttn[lix](x=x_que, xpos=que_pos, y=x_ref,
                                                    ypos=ref_pos)  # BxVqHWxC -> BxVqHWxC
    ...
    def generate_rotation_aware_embedding(self, x, mask):
        ...
        x: BxCxSxS
        """
        # RA-Encoder includes the DINOv2 backbone, four 3 × 3 convolutional layers with stride 2, a generalized
        # average pooling layer, and a fully connected layer with an output dimension of 64 (see Fig. 3 bottom).
        assert (x.dim() == 4), 'x: {}'.format(x.shape)
